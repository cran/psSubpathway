...
##' @param fdr.th Cutoff value for fdr. Only subpathway with lower fdr are ploted. (default: 1).
##' @param SES Parameter `SES` is useful only when `plotSubSEA` is TRUE. When `plotSubSEA=TRUE`,if `SES` is positive,
##' the subpathway with high-expression will be ploted.when it is negative, plot low-expression subpathways.
##' @param phentype Parameter `phentype` is useful only when `plotSubSEA` is TRUE. `phentype` decides which phenotypic
##' significant subpathways to screen (which phenotypic result is applied to parameter `fdr.th` and `SES`.) and plot a
##' heat map of these subpathways.By default,`phentype="all"` which will screen the subpathways of all phenotypes and plot
...
##' @export
plotheatmap<-function(inputdata,plotSubSEA=TRUE,fdr.th=1,SES="positive",phentype="all"){
  if(plotSubSEA==TRUE){
    ...
    if(SES=="positive"){
      ...
      if(phentype=="all"){
        ...
        rowann = data.frame(Subpathway = factor(rep(paste(phentype,"-specific",sep = ""),length(rtmatrix[,1]))))
        rownames(rowann) <-row.names(rtmatrix)
        colann=data.frame(Sample=factor(rep(names(table(colnames(rtmatrix))),
                                                 table(colnames(rtmatrix)))))
        samples<-paste(colnames(rtmatrix),1:length(rtmatrix[1,]))
        ...
        pheatmap(rtmatrix,color = colorRampPalette(c("navy", "white", "firebrick3"))(50),
                 ...
                 annotation_row =rowann,annotation_col =colann,
                 show_rownames=T,show_colnames=F,
                 main=paste(phentype,"specific up-regulation subpathway heat map"))
    ...
    if(SES=="negative"){
      ...
      if(phentype=="all"){
        ...
        pheatmap(rtmatrix,color = colorRampPalette(c("navy", "white", "firebrick3"))(50),
                 ...
                 annotation_row =rowann,annotation_col =colann,
                 show_rownames=T,show_colnames=F,
                 main=paste(phentype,"specific down-regulation subpathway heat map"))
    ...
    if(SES=="all"){
      ...
      if(phentype=="all"){
        ...
        pheatmap(rtmatrix,color = colorRampPalette(c("navy", "white", "firebrick3"))(50),
                 ...
                 annotation_row =rowann,annotation_col =colann,
                 show_rownames=T,show_colnames=F,
                 main=paste("All",phentype,"specific subpathway heat map"))
